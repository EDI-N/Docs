...
.. note:: Наличие ритейлера при документообороте автоматически определяется платформой (т.е. если Отправитель или Получатель является ритейлером, то созданная цепочка будет иметь пометку конкретного ритейлера)
